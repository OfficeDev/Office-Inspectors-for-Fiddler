...
namespace MAPIInspector.Parsers
{
	...
	#region Fasttransfer stream need rops in other protocol: this will be delete after other protocol has added
	public class RopCreateFolderResponse : BaseStructure
    {
        ...
        public override void Parse(Stream s)
        {
            ...
            this.IsExistingFolder = ReadByte();
            if (this.IsExistingFolder != 0)
            { 
            this.HasRules = ReadByte();
            this.IsGhosted = ReadByte();
            }
            if (this.IsExistingFolder != 0 && this.IsGhosted != 0)
            {
                this.SeverCount = ReadUshort();
                this.CheapServerCount = ReadUshort();
                for (int i = 0; i < this.SeverCount; i++)
                {
                    this.Servers[i] = ReadString();
                }
            }
        }
    }
